...
function toDeepSeekMessages({ instructions, messages, inputItems }) {
  const finalMessages = [];
...
function createDeepSeekCompletionRequestBody({
  model,
  instructions,
  messages,
  tools,
...
function extractDeepSeekMessage(payload) {
  return payload?.choices?.[0]?.message || null;
}

function extractDeepSeekFunctionCallsFromMessage(message) {
...
function extractDeepSeekAssistantBlocksFromMessage(message) {
  const blocks = [];
...
async function getXAiChatCompletionStream({
  ...
  const toolCalls = extractDeepSeekFunctionCallsFromMessage(message);
  const assistantBlocks = extractDeepSeekAssistantBlocksFromMessage(message);
