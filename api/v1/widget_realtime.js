...
const FOLLOWUP_MODEL = "gpt-realtime-mini";
...
async function getXAiChatCompletionStream({
  ...
  if (!response.ok) {
    ...
    return { ok: false, status: response.status || 502, error: errText || "OpenAI request failed" };
  }
  ...
  if (!rawText) {
    return { ok: false, status: 502, error: "Empty model output from OpenAI" };
  }
