...
const { rerankVoyageDocuments } = require("../../scripts/internal/rerankVoyageDocuments");
    ...
    normalizedMessage && !SKIP_VECTOR_MESSAGES.has(normalizedMessage)
      ? (async () => {
          ...
          if (!skipRerank && filteredVectorCandidates.length > 1) {
            ...
            rerankUsed = true;
            const rerankedItems = (Array.isArray(rerankResult.results) ? rerankResult.results : [])
              .map((item) => {
            ...
            rerankCandidates = rerankedItems;
          }
