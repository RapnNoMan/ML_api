...
const PHONE_COUNTRY_BY_CALLING_CODE = Object.freeze({
  "20": "EG",
  "27": "ZA",
  "30": "GR",
  "31": "NL",
...
function inferCountryFromPhone(value) {
  ...
  for (const length of [3, 2, 1]) {
    ...
    const country = PHONE_COUNTRY_BY_CALLING_CODE[code];
    if (country) return country;
  }
  return null;
}

function getChannelCustomerCountry({ event }) {
  if (event?.channel === "whatsapp") {
    return inferCountryFromPhone(event.senderId);
  }
...
async function processIncomingMessage({
  ...
  const requestCountry = getChannelCustomerCountry({ event });
