...
function inputItemContentToText(content) {
...
function toGeminiMessages({ instructions, messages, inputItems }) {
  const finalMessages = [];
  const systemText = String(instructions || "").trim();
  if (systemText) finalMessages.push({ role: "system", content: systemText });

  const items = Array.isArray(inputItems) ? inputItems : toOpenAiInputItems(messages);
  for (const item of items) {
    if (item?.type === "message") {
      const role = item?.role === "assistant" ? "assistant" : "user";
      finalMessages.push({
        role,
        content: inputItemContentToText(item?.content),
      });
      continue;
    }
    if (item?.type === "function_call") {
      finalMessages.push({
        ...
        tool_calls: [
          {
            id: String(item?.call_id || `call_${finalMessages.length}`),
            type: "function",
            function: {
              name: String(item?.name || ""),
...
function createGeminiCompletionRequestBody({
  model,
  instructions,
  messages,
  tools,
...
function extractGeminiFunctionCallsFromMessage(message) {
  ...
  return toolCalls
    .map((toolCall) => {
      ...
      return {
        action_key: String(toolCall?.function?.name || ""),
        variables,
        call_id: toolCall?.id ?? null,
      };
    })
    .filter((call) => call.action_key);
}

function extractGeminiAssistantBlocksFromMessage(message) {
  const blocks = [];
  ...
  if (!apiKey) {
    return {
      ...
      error: fallbackReason
        ? `Gemini failed and Grok fallback is not configured: ${fallbackReason}`
...
async function getXAiChatCompletionStream({
  ...
  const requestBody = createGeminiCompletionRequestBody({
    model,
    instructions,
    messages,
    tools,
  ...
  let response;
  try {
    response = await fetch(GEMINI_OPENAI_CHAT_COMPLETIONS_API_URL, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    ...
    return getGrokChatCompletionStream({
      ...
      signal,
      onTextDelta,
      fallbackReason: errText || "Gemini request failed",
    });
  }
  ...
  if (!response.body) {
    return getGrokChatCompletionStream({
      ...
      signal,
      onTextDelta,
      fallbackReason: "Missing stream body from Gemini",
    });
  }
  ...
  while (!done) {
    ...
    try {
      ...
      if (assembledText) {
        return { ok: false, status: 502, error: "Stream read error from Gemini" };
      }
      return getGrokChatCompletionStream({
        apiKey: fallbackApiKey,
        model: fallbackModel,
        ...
        signal,
        onTextDelta,
        fallbackReason: "Stream read error from Gemini",
      });
    }
    done = Boolean(chunk?.done);
    if (chunk?.value) {
      ...
      while (delimiterIdx !== -1) {
        ...
        const payload = evt.payload;
        if (payload?.error) {
          upstreamError = payload.error?.message || "Gemini streaming failed";
        }
        ...
        for (const toolCallDelta of Array.isArray(delta?.tool_calls) ? delta.tool_calls : []) {
          const index = Number.isFinite(Number(toolCallDelta?.index))
          ...
          if (toolCallDelta?.id) existing.id = String(toolCallDelta.id);
          if (toolCallDelta?.type) existing.type = String(toolCallDelta.type);
          if (toolCallDelta?.function?.name) {
            existing.function.name += String(toolCallDelta.function.name);
          }
  ...
  const message = {
    ...
    tool_calls: Array.from(streamedToolCalls.values()).filter(
      (toolCall) => toolCall?.function?.name
    ),
  };
  const toolCalls = extractGeminiFunctionCallsFromMessage(message);
  const assistantBlocks = extractGeminiAssistantBlocksFromMessage(message);
  ...
  if (!rawText) {
    return getGrokChatCompletionStream({
      ...
      signal,
      onTextDelta,
      fallbackReason: "Empty model output from Gemini",
    });
  }
...
async function getXAiChatCompletion({
  ...
  try {
    ...
    return getGrokChatCompletion({
      ...
      tools,
      inputItems,
      fallbackReason: "Invalid JSON from Gemini",
    });
  }
